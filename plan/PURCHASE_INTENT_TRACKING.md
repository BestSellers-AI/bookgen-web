...
Tracks every checkout attempt (click on "Subscribe" or "Buy Credits") as a `PurchaseIntent` record. If the user doesn't complete the purchase within 30 minutes, a recovery email is sent automatically. Admin panel shows all intents with conversion stats.
...
model PurchaseIntent {
  ...
  stripeSessionId     String?   // links intent to Stripe session
...
|--------|----------|-------------|
| `landing` | LP pricing section | Any plan or credit pack CTA |
...
1. Queries intents where:
   ...
   - `createdAt` between 30 minutes and 24 hours ago
2. Sends localized recovery email (different templates for plans vs credits)
3. Marks `recoveryEmailSentAt` to prevent duplicates
4. Maximum 100 emails per run

**Timing:**
- < 30 min: too early, user might still be on Stripe
- 30 min – 24h: recovery window
- > 24h: too late, not sent

**Email templates (3 locales: en, pt-BR, es):**
- Subscription recovery: "Still thinking about your plan?"
- Credit pack recovery: "Your credits are waiting"
- Both link to the pricing/upgrade page
...
- Clicking a plan when already subscribed (plan change via API, not Stripe)

## Admin Panel

### Sidebar
New item "Purchase Intents" (ShoppingCart icon) in the admin section.
...
- Recovery (email sent date, or "Pending")
...
| `cron/cron.service.ts` | Hourly abandonment detection + recovery emails |
...
| `admin/admin.service.ts` | `listPurchaseIntents()` + `getPurchaseIntentStats()` |
...
| `lib/api/admin.ts` | Added types + API functions for intents |
...
| `app/[locale]/dashboard/admin/purchase-intents/page.tsx` | New admin page |
...
User clicks "Subscribe" → PurchaseIntent created → Stripe redirect
  ...
  └─ User abandons → 30min passes → cron detects → recovery email sent
